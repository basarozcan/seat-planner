...
export default new Vuex.Store({
    ...
    actions: {
        async addBoard({ commit, dispatch }, payload){
            await axios.post(`${process.env.VUE_APP_API_URL}/board`, {'name': payload.name, 'owner_id': payload.owner_id })
            dispatch('fetchBoards')
        },
        
        async addTable({ commit, dispatch }, payload){
            await axios.post(`${process.env.VUE_APP_API_URL}/table`, {'name': payload.name, 'board': payload.board })
        ...
        async fetchTables({ commit, state }) {
            const result = await axios.get(`${process.env.VUE_APP_API_URL}/board/${state.selectedBoard._id}/get-tables/json`)
        ...
        async fetchUser({ commit }){
            await axios.get(`${process.env.VUE_APP_API_URL}/auth/check`, {
        ...
        async fetchBoards({commit, state}){
            const result = await axios.get(`${process.env.VUE_APP_API_URL}/board/all/${state.user._id}/json`, {
                headers: { "Content-Type": "application/json" },
                withCredentials: true
            })
        ...
        async fetchBoardData({commit, dispatch}, payload){
            await axios.get(`${process.env.VUE_APP_API_URL}/board/${payload.id}/json`, {
                headers: { "Content-Type": "application/json" },
                withCredentials: true
            })
            .then(res => {
        ...
        async logoutUser({commit}){
            const result = await axios.get(`${process.env.VUE_APP_API_URL}/auth/logout`, {
                headers: { "Content-Type": "application/json" },
                withCredentials: true
            })
